...
int main(int argc, char *argv[]) {
    if (argc < 4) {
        fprintf(stderr, "Usage: %s <server_port> <test_cases> <output_file>\n", argv[0]);
        exit(EXIT_FAILURE);
    }
    ...
    send_packet.ack = cur_ack;
    send_packet.flag = ACK;
    sprintf(send_packet.payload, "%d", window_size);
    send_packet.payload_size = strlen(send_packet.payload);
    sendto(sockfd, &send_packet, sizeof(Packet), 0,
           (struct sockaddr *)&server_addr, addrlen);
    printf("Sent window size (N) = %d\n", window_size);

    send_packet.seq = ++cur_seq;
    sprintf(send_packet.payload, "%d", byte_request);
    send_packet.payload_size = strlen(send_packet.payload);
    sendto(sockfd, &send_packet, sizeof(Packet), 0,
           (struct sockaddr *)&server_addr, addrlen);
        ...
        default:
            ...
            if ((recv_packet.seq == cur_ack + 1) && (recv_packet.ack == cur_seq)) {
                ...
                case ACK:
                    ...
                    if (recv_packet.payload_size > 0) {
                        fwrite(recv_packet.payload, 1, recv_packet.payload_size, output_file);
                        printf("Wrote %zu bytes to file\n", recv_packet.payload_size);
                    }
